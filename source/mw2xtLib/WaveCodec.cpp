...
WaveData decodeWave(const std::vector<uint8_t>& nibbles128) {
    if (nibbles128.size() != 128) return WaveData{};
    WaveData wave{};
    for (int i = 0; i < 64; ++i) {
        const uint8_t high = nibbles128[i * 2];
        const uint8_t low = nibbles128[i * 2 + 1];
        int sample = (high << 4) | low;
        sample ^= 0x80;
        wave[i]       = static_cast<int8_t>(static_cast<uint8_t>(sample));
        wave[127 - i] = static_cast<int8_t>(static_cast<uint8_t>(-sample));
    }
    return wave;
}
